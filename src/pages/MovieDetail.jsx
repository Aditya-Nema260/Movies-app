...
const MovieDetail = () => {
  ...
  const title = detail.title || detail.original_name;
      ...
      <div className="relative z-10 p-4 md:p-16 text-white">
        <div className="flex flex-col md:flex-row items-start space-y-8 md:space-y-0 md:space-x-12">
          <img
            src={posterImageUrl}
            alt={title}
            className="w-64 md:w-80 rounded-lg shadow-2xl flex-shrink-0"
          />
          ...
          <div className="flex-grow">
            ...
            <p className="text-xl text-gray-300 mb-6 font-medium">
              ...
              {releaseDate} | {"Run Time - "}
              {runtime}m
            </p>

            {detail.tagline && (
              <p className="text-2xl text-amber-400 mb-6">"{tagLine}"</p>
            )}
            ...
            {detail.genres && (
              <div className="flex flex-wrap gap-3 mb-6">
                {detail.genres.map((g) => (
                  <span
                    key={g.id}
                    className="bg-red-600 hover:bg-red-700 px-4 py-1 rounded-full text-sm font-semibold transition duration-300"
                  >
                    {g.name}
                  </span>
                ))}
            ...
            <button
              className="cursor-pointer bg-teal-500 text-black font-extrabold py-3 px-8 rounded-full transition-transform duration-300 hover:bg-teal-400 hover:scale-105 shadow-xl"
            ...
            <Link to={homePage}>
              <button
                className=" cursor-pointer  bg-teal-500 text-black m-2 font-extrabold py-3 px-8 rounded-full transition-transform duration-300 hover:bg-teal-400 hover:scale-105 shadow-xl"
                onClick={() => dispatch(addToFavourite(detail))}
              >
                More Details
              </button>
